...
func (gq *GenderQuery) sqlAll(ctx context.Context) ([]*Gender, error) {
	...
	if query := gq.withUsers; query != nil {
		...
		for _, n := range neighbors {
			fk := n.gender_users
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "gender_users" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "gender_users" returned %v for node %v`, *fk, n.ID)
			}
