...
func (u *User) assignValues(values ...interface{}) error {
	...
	if value, ok := values[1].(*sql.NullString); !ok {
		return fmt.Errorf("unexpected type %T for field idemployee", values[1])
	} else if value.Valid {
		u.Idemployee = value.String
	}
	values = values[2:]
	if len(values) == len(user.ForeignKeys) {
		if value, ok := values[0].(*sql.NullInt64); !ok {
			return fmt.Errorf("unexpected type %T for edge-field gender_users", value)
		} else if value.Valid {
			u.gender_users = new(int)
			*u.gender_users = int(value.Int64)
		}
	}
	return nil
}
...
func (u *User) String() string {
	...
	builder.WriteString(u.Idemployee)
	builder.WriteByte(')')
